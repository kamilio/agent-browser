...
| --- | --- | --- | --- |
| K01 | HTML parsing, DOM identity and mutation | Malformed markup, entities, trees, selectors and mutation fixtures; relevant web-platform tests. | Partial parser, identity, bounded queries, fragments, cloning, live tag/class/children collections and Attr/NamedNodeMap (`HTML.md`, `DOM-FRAGMENTS.md`, `LIVE-COLLECTIONS.md`, `DOM-ATTRIBUTES.md`); namespace/prototype completeness, full collection/NodeList, parser/DOM conformance and web-platform coverage remain open |
...
| P15 | `route`, `route-list`, `unroute` | Requests actually mocked, intercepted bodies/status and retrieval showing those results. | Partial session-owned fulfillment, bounded URL globs, response bodies/status/headers, listing/removal and journal attribution (`ROUTING.md`). Real HTML/styles and existing-core interpreted fetch use replacement content in memory. Automatic transport redirects fail closed while rules are active; header rewriting, binary/redirect/cookie mocks, handlers, full pattern parity and separate CLI/live acceptance remain open |
