...
| K09 | Interactive input and selection | Mouse/keyboard events, focus, links, forms and document selection driven through the API. | Partial native double-click/default actions, guarded focus, shared live Range/Selection, editable fill/type/paragraph split/merge, bounded glyph/terminal carets and same-text selection highlighting (`DOUBLE-CLICK.md`, `FOCUS-BRIDGE.md`, `DOM-RANGES.md`, `RANGE-GEOMETRY.md`, `EDITABLE-CARET.md`, `EDITABLE-CARET-BREAKS.md`, `EDITABLE-SELECTION.md`, `EDITABLE-BLOCK-MERGE.md`). Native host/capture fixtures cover cancellation, ownership and fixed/root scroll. Mixed-node/full visual selection, IME/control selection, complex editing and released-runtime/live acceptance remain open |
