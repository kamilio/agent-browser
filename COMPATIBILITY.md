...
The 143 new cases and 781 / twenty-one-file focused suites pass in both trees;
twelve native capture phases preserve state/boxes and match both builds exactly.
Full public control selection APIs, pointer placement, persistent widget scrolling,
platform rendering and original browser/runtime/live gates remain open.
...
actual session fetch port pass focused checks; custom-provider unknown traffic,
future-allocation reservations, RSS, full-suite authorization, XHR and original
browser/runtime/live gates remain open. Earlier checkpoint evidence is preserved.

September 4 element-boundary editing continuation: `EDITABLE-ELEMENT-ENDPOINTS.md`
adds child-slot selection painting and exact focused block-host outer-edge carets
through plain inline text paths. Native select-all/arrow/cancel/type, scrolling and
software PNG comparisons preserve public Range state. General element caret
geometry, controls/IME/full visual selection and external/runtime gates remain open.
...
replacement, direction, cancellation and fixed/root scrolling have fresh tests
and captures. Element/control endpoints and full visual/runtime gates remain open.
...
tests and inspected captures. Mixed-node/control selection, general visual editing
and original external/runtime gates remain open; K09 is still partial.
...
complete platform focus, shadow/modal behavior or released-runtime parity.
...
| K05 | Browser network APIs | Fetch, XHR, URL/encoding, redirects and CORS with local multi-origin security fixtures. | Partial document-owned fetch (`PAGE-FETCH.md`) plus CORS/preflights/header filtering/redirect state (`PAGE-CORS.md`), historically tested with real SafeJS over mock origins. Later receiving ceilings and live page byte/drain accounting (`FETCH-BYTE-ACCOUNTING.md`) have native in-memory validation only. XHR, binary/streaming/signals, broader conformance and real multi-origin/wire acceptance remain open |
...
| K09 | Interactive input and selection | Mouse/keyboard events, focus, links, forms and document selection driven through the API. | Partial native double-click/default actions, guarded focus, shared live Range/Selection, editable fill/type/paragraph split/merge, bounded glyph/terminal carets and text/element-endpoint highlighting and bounded host-edge carets (`DOUBLE-CLICK.md`, `FOCUS-BRIDGE.md`, `DOM-RANGES.md`, `RANGE-GEOMETRY.md`, `EDITABLE-CARET.md`, `EDITABLE-CARET-BREAKS.md`, `EDITABLE-SELECTION.md`, `EDITABLE-SELECTION-MIXED.md`, `EDITABLE-ELEMENT-ENDPOINTS.md`, `EDITABLE-BLOCK-MERGE.md`). Native host/capture fixtures cover cancellation, ownership and fixed/root scroll. General caret geometry/full visual selection, IME/control selection, complex editing and released-runtime/live acceptance remain open |
...
| P04 | `snapshot`, `find` | Whole/scoped/depth/boxes snapshots, files, refs, literal/regex searches. | Partial native snapshots and streaming bounded literal/regex find with refs, context and truncation metadata (`SNAPSHOT-SEARCH.md`). Search reaches beyond the former 10,000-entry/1 MiB cutoff while preserving output/work/owner bounds. Shared-host/actual experimental-core checks pass. Full regex, boxes/files, separate CLI and upstream parity acceptance remain open |
