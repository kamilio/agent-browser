...
async function fixture(css = "", attributes = "", extra = "") {
	...
	const bodies = new Map([
		[
			...
			`<!doctype html><title>Start</title><style>html,body{margin:0;padding:0;font-family:'Agent Mono';font-size:8px;line-height:8px}main{width:100px;letter-spacing:4px}${css}</style><main><a id="target" href="/next" ${attributes}>ABC</a></main>${extra}`,
		],
		[
...
it("navigates a discovered spaced link only after receiving pointer events", async () => {
...
it("does not navigate when a spaced link cancels its click", async () => {
...
it.each([
	...
	"keeps a %s spaced link non-actionable",
