...
export interface StylesheetFetchPolicy {
	readonly mode: "cors" | "no-cors";
	readonly credentials: FetchCredentials;
}

export interface StylesheetFetchContext {
...
function target(input: string, base: string, document: URL): URL {
	...
	if (document.protocol === "https:" && url.protocol !== "https:")
		throw new AgentBrowserError(
			"policy-denied",
			"Mixed-content stylesheet fetch is not allowed",
		);
	url.hash = "";
	return url;
}
...
function responseHeaders(input: NetworkResponse["headers"]) {
	...
	for (const [key, values] of Object.entries(input)) {
		if (key.length > 256 || !headerToken.test(key) || !Array.isArray(values))
			throw new TypeError("Invalid stylesheet response header");
		...
		for (const value of values) {
			...
			if (bytes > 16_384)
				throw new AgentBrowserError(
					"resource-limit",
					"Stylesheet response header limit exceeded",
				);
...
export async function fetchStylesheetResource(
	input: string,
	...
	for (;;) {
		...
		const response = await request({
			url: url.href,
			method: "GET",
			headers: { accept: "text/css", ...(cors ? { origin } : {}) },
			redirect: "manual",
			signal,
			cookieContext: {
				siteUrl: documentUrl,
		...
		if (redirectStatuses.has(response.status) && locations !== undefined) {
			if (locations.length !== 1)
				throw new AgentBrowserError(
					"network-error",
					"Ambiguous stylesheet redirect location",
				);
			if (redirects.length >= maxRedirects)
				throw new AgentBrowserError(
					"resource-limit",
					"Stylesheet redirect limit exceeded",
				);
