...
export function isHtmlParserNode(node: HtmlParserNode, tag?: string): boolean {
	return (
		node.tree.namespaceOf(node.id) === htmlNamespace &&
...
function hasCategory(
	...
	const namespaceURI = node.tree.namespaceOf(node.id);
	...
	add(node: HtmlParserNode, attributes: Record<string, string>) {
		...
		for (let index = this.entries.length - 1; index >= 0; index--) {
			...
			if (
				...
				entry.node.tree.namespaceOf(entry.node.id) ===
					node.tree.namespaceOf(node.id) &&
